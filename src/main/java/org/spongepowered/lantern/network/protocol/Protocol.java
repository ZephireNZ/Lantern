...
public abstract class Protocol extends AbstractProtocol {
    ...
    public <M extends Message> MessageHandler<?, M> getMessageHandle(Class<M> clazz) {
        ...
        if (handler == null) {
            SpongeImpl.getLogger().warn("No message handler for: " + clazz.getSimpleName() + " in " + getName());
        }
        return handler;
    }
    ...
    public <M extends Message> Codec.CodecRegistration getCodecRegistration(Class<M> clazz) {
        ...
        if (reg == null) {
            SpongeImpl.getLogger().warn("No codec to write: " + clazz.getSimpleName() + " in " + getName());
        }
        return reg;
    }
